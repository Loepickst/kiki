(function() {
    ...
    function createQuestionMap(type, level) {
        ...
        getPool(type, level).forEach((question) => {
            ...
            const questionId = Number.parseInt(question.questionId, 10);
            if (!examKey || !Number.isInteger(questionId)) return;
    ...
    function normalizeItem(item) {
        ...
        const questionId = Number.parseInt(item.questionId || item.id, 10);
        if (!sourceExamKey || !Number.isInteger(questionId) || questionId < 1) {
            return null;
        }
        return {
            sourceExamKey,
    ...
    function buildCategoryPracticeUrl(config) {
        ...
        if (config && config.questionId) {
            params.set('questionId', String(Number.parseInt(config.questionId, 10)));
        }
    ...
    function parseStoredQuestion(rawValue) {
        try {
            ...
            const questionId = Number.parseInt(parsed.questionId || parsed.id, 10);
            if (!sourceExamKey || !Number.isInteger(questionId) || questionId < 1) {
                return null;
            }
    ...
    function createQuestionSignature(question) {
        ...
        const questionId = Number.parseInt(question && (question.questionId || question.id), 10);
        if (!sourceExamKey || !Number.isInteger(questionId) || questionId < 1) return '';
