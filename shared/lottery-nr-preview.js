(function(window){
 ...
 const rendererUrl=new URL('nr-holographic/renderer.classic.js?v=20260922-5',scriptUrl).href;
 const assetBase=new URL('../assets/nr-holographic/',scriptUrl).href;
 function loadRenderer(){
  if(window.KikiNRRenderer)return Promise.resolve(window.KikiNRRenderer);
  if(loading)return loading;
  loading=new Promise((resolve,reject)=>{
   const script=document.createElement('script');script.src=rendererUrl;script.async=true;
   script.onload=()=>{if(window.KikiNRRenderer)resolve(window.KikiNRRenderer);else{script.remove();loading=null;reject(new Error('NR renderer unavailable'));}};
 ...
 async function show(container,card){
  ...
  try{const effect=await loadRenderer();if(token!==sequence)return;document.getElementById('previewCard').classList.add('is-nr-holo');dispose=effect.mount(target,card,{assetBase});}catch(error){console.error('[NR preview]',error);if(token===sequence)clear();}
 }
