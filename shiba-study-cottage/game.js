(function(C){
  ...
  class Game {
    constructor(canvas,store,audio){
      ...
      this.relationship=new C.Relationship(store,()=>this.onSave(true));this.careContext=null;this.petResponse=null;this.invitationFeedback=null;this.treatRequest=null;this.sleepSession=null;this.windowThought=null;this.nextThoughtAt=0;this.thoughtRandom=Math.random;
    ...
    bind(){
      ...
      window.addEventListener('pagehide',()=>{this.pendingTarget=null;this.cancelBark();this.needs.flush();this.relationship.flush();});
      ...
      document.addEventListener('visibilitychange',()=>{this.suspended=document.hidden;if(document.hidden){this.windowThought=null;this.pendingTarget=null;this.cancelBark();this.needs.flush();this.relationship.flush();}this.takeControl();if(document.hidden)this.stopRoute();this.audio.syncMusic();});
      ...
      this.canvas.addEventListener('contextmenu',e=>{e.preventDefault();if(this.build?.id)this.cancelPlacement();});
    ...
    goToTarget(id){
      ...
      if(target.action==='snacks'&&this.needs.levels.hunger<20){this.onToast('肚子还饱着，晚一点再吃吧。');return false;}
    ...
    interactionLabel(target){
      ...
      if(target.id==='snackRack')return '零食架 · 口粮与小零食';
      ...
      const actions={shop:'选购家具',sleep:'休息一会儿',drink:'喝口水',play:'邀请玩一会儿',snacks:this.needs.levels.hunger<20?'肚子还饱着':'吃一份口粮',window:'看看窗外',lamp:'开关暖灯',music:'播放 / 暂停'};
    ...
    offerTreat(item,id){
      if(this.blocked||this.build||this.suspended)return false;
      if(this.treatRequest||this.finishingAction){this.onToast('等它做完现在的动作，再给它尝尝吧。');return false;}
      const reason=C.Snacks.canOffer(this.store.state,item,this.needs.levels);if(reason){this.onToast(reason);return false;}
      if(this.isResting){
        const target=this.targets.find(t=>t.id==='snackRack');
        if(!target||!M.findPath(this.placements,this.player,target.anchor)){this.onToast('先摆好能走到的零食架，再用小零食唤醒它。');return false;}
      ...
      if(!this.goToTarget('snackRack'))return false;
      if(this.arrivalTarget!=='snackRack')return false;
      this.treatRequest={id,item,phase:'walking'};this.onStatusChange?.();return true;
    }
    beginTreat(target){
      const request=this.treatRequest;if(!request)return false;
      ...
      const outcome=this.store.state.snacks?.tastes[request.item];
      if(!outcome){this.clearAction();return false;}
      const ok=this.perform(outcome==='dislike'?'sniff':'eat',target);
      if(ok){this.treatRequest={...request,phase:'tasting'};this.onStatusChange?.();}return ok;
    }
    finishTreat(){
      ...
      try{
        ...
        const texts={like:'它吃完后摇了摇尾巴，很喜欢这个味道。',neutral:'它慢慢吃完了，觉得这个味道还不错。',dislike:'它闻了闻，转过了头，似乎不太喜欢。零食收回架上了。'};
    ...
    pause(value){if(this.blocked===value)return;if(value){this.windowThought=null;this.invitationFeedback=null;this.pendingTarget=null;this.cancelBark();}this.takeControl();this.blocked=value;if(value)this.stopRoute();this.updateHover();}
    normalizePlayer(){
      this.refreshWorld();
    ...
    update(dt){
      ...
      if(['drink','eat'].includes(this.action)){const live=this.targets.find(t=>t.id===this.actionTarget?.id);if(!live||Math.hypot(live.anchor.x-this.player.x,live.anchor.y-this.player.y)>.5)this.clearAction();}
      ...
      if(this.destination){
        ...
        const walkingTreat=this.treatRequest;
        this.clearAction();
        if(walkingTreat?.phase==='walking')this.treatRequest=walkingTreat;
        ...
        if(this.destination&&!this.route.length){
          const id=this.arrivalTarget,automatic=this.idleLife.active,treat=this.treatRequest;this.stopRoute();this.clearAction();
          if(id==='snackRack'&&treat?.phase==='walking')this.treatRequest=treat;
    ...
    endSleep(){
      ...
      if(treat){
        const target=this.targets.find(t=>t.id==='snackRack'),reason=C.Snacks.canOffer(this.store.state,treat.item,this.needs.levels);
        if(!reason&&target&&this.walkTo(target.anchor,target.id))this.treatRequest={...treat,phase:'walking'};
        else this.onToast(reason||'暂时走不到零食架，零食还留在架上。');
      }
      this.onStatusChange?.();
    }
    finishPlay(){
    ...
    perform(action,target,automatic=false){
      ...
      if(['drink','eat'].includes(action)){
        ...
        const live=this.targets.find(t=>t.id===target?.id&&t.action===(action==='eat'?'snacks':'drink'));
    ...
    beginBuild(){if(this.blocked)return;if(this.isResting){this.sleepingNotice();return;}this.pendingTarget=null;this.takeControl();this.stopRoute();this.clearAction();this.history=[];this.build={id:null,candidate:null,check:null};this.onBuildChange();this.canvas.focus();}
    ...
    render(){
      ...
      layers.push({y:this.renderPosition&&this.actionTarget?((this.placements.find(i=>i.id===this.actionTarget.id)?.y||0)+(F[this.actionTarget.id]?.h||0))*16+1:p.y,draw:()=>{
        ...
        if(action==='eat'){
          // A temporary serving belongs to the interaction, not the dog atlas.
          A.oval(c,Math.round(drawX)+6,Math.round(drawY)-3,12,5,'shadow');
          A.rect(c,Math.round(drawX)+7,Math.round(drawY)-5,10,5,'greenDark');
          A.rect(c,Math.round(drawX)+8,Math.round(drawY)-5,8,2,'cream');
          A.rect(c,Math.round(drawX)+9,Math.round(drawY)-5,2,1,'woodShadow');A.rect(c,Math.round(drawX)+13,Math.round(drawY)-5,2,1,'woodShadow');
        }
