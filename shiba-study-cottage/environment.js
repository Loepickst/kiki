(function(C){
  ...
  function sample(date=new Date()){
    ...
    const civil=new Date(date.getTime()+28800000),hour=civil.getUTCHours()+civil.getUTCMinutes()/60;
    ...
    return {key:Math.floor(date.getTime()/60000)+':'+date.getTimezoneOffset(),hour,label,night,roomLampOn:hour>=C.Data.roomLamp.onHour||hour<C.Data.roomLamp.offHour,
      clock:[civil.getUTCHours(),civil.getUTCMinutes()].map(v=>String(v).padStart(2,'0')).join(':'),
  ...
  // Rendering-only exposure. Keep the civil-time / sunlight values above stable:
  // sleep, sheltered rest and every saved need continue to use the same inputs.
  // Multiplying a cool tint retains wood/grass chroma instead of putting a grey
  // source-over veil across the scene. Cloud exposure fades at night so the
  // weather and night passes cannot independently darken the same scene twice.
  function lightProfile(state,weather=null){
    const clamp=n=>Math.max(0,Math.min(1,Number(n)||0));
    const night=clamp(state.darkness/.32),sun=clamp(state.sunlight);
    const cloud=clamp(weather?.cloud),rain=clamp(weather?.rain);
    return Object.freeze({
      night,sun,
      roomShade:night*.68,
      yardShade:night*.76,
      cloudShade:(cloud*.12+rain*.035)*(1-night*.82),
      duskWarmth:clamp(state.warmth)*.3,
      lampWarmth:.08+night*.07,
      roomTint:'#8da1be',yardTint:'#8196b7',cloudTint:'#aebec9'
    });
  }
  C.Environment={sample,Clock,color,inSunbeam,lightProfile};
})(globalThis.Cottage);
