(function(C){
  ...
  class Relationship {
    constructor(store,onSaved=()=>{},random=Math.random,now=()=>Date.now()){this.store=store;this.onSaved=onSaved;this.random=random;this.now=now;this.data=clone(R.initial);this.baseline=null;this.dirty=false;this.elapsed=0;this.sync();}
    sync(){
    ...
    get stage(){
      this.sync();const s=this.data,c=this.store.state.care;let rank=C.PetCare.rank(s);
      while(c&&rank>(c.legacyRank||0)&&c.activeDays<[0,2,7,14,21][rank])rank--;
      if(rank===0)return {id:'wary',rank:0,label:'有点戒备',description:'它需要一点距离。先照顾需要，让它慢慢愿意靠近。'};
      if(rank===1)return {id:'new',rank:1,label:'慢慢熟悉',description:'它正在认识你，短短的温柔互动就很好。'};
      if(rank===2)return {id:'familiar',rank:2,label:'安心相处',description:'在你身边更放松，愿意接受摸摸。'};
      if(rank===3)return {id:'close',rank:3,label:'喜欢靠近',description:'有时会主动走到前方，想和你待在一起。'};
    ...
    update(dt,quiet=true){
      this.sync();const s=this.data;s.clock=Math.max(s.clock,this.now()/1000);
    ...
    record(kind){
      if(!['boundary','space'].includes(kind))return false;
      this.dirty=true;if(!this.flush())return false;
      const at=this.now(),e=C.CareEvent(kind,true,null,{},at);
      try{this.store.commit(C.PetCare.settle(C.PetCare.advance(this.store.state,at),e,at));this.sync();this.onSaved();return true;}catch{this.sync();return false;}
    }
    decline(reason){
      const s=this.data;
    ...
    requestPet(action){
      this.store.petCare?.flush();this.sync();const s=this.data;s.clock=Math.max(s.clock,this.now()/1000);
      ...
      s.comfort=Math.max(0,s.comfort-2);s.repairAt=0;this.dirty=true;if(!this.flush())return {accepted:false,message:'这次互动还没有保存，请稍后再试。'};
    ...
    requestInvitation(action,context={}){
      ...
      this.store.petCare?.flush();this.sync();const s=this.data,n={...C.Data.needs.initial,...context.needs},t=C.Data.needs.thresholds;s.clock=Math.max(s.clock,this.now()/1000);
    ...
    complete(kind){
      return false; // Finished interactions settle atomically through PetCare.
    }
  }
